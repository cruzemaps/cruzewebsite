...
@media (prefers-reduced-motion: no-preference) {
  .lane-div-track { animation: lane-scroll 3s linear infinite; }
}
`;
...
export default function LaneDivider() {
  useInjectCSS();
  return (
    <div aria-hidden style={{ position: "relative", height: 30, overflow: "hidden", background: "#0A0C10", borderTop: "1px solid rgba(255,255,255,0.05)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
      <div className="lane-div-track" style={{ position: "absolute", top: "50%", left: 0, width: "200%", height: 3, display: "flex" }}>
        {[0, 1].map((k) => (
          <div key={k} style={{ width: "50%", height: "100%", background: "repeating-linear-gradient(90deg, rgba(233,177,58,0.6) 0 32px, transparent 32px 72px)" }} />
        ))}
      </div>
    </div>
  );
